...
\begin{definition}
有穷公式序列$\varphi_1,\,\cdots,\,\varphi_n$表示从$\Sigma$到$\varphi$的一个\indexbf{推演}, 如果其中的任意$\varphi_i$要么是属于$\Sigma$的, 要么可从之前的公式$\varphi_j$和$\varphi_k=\varphi_j\to\varphi_i$得到, 而且$\varphi_n=\varphi$. 记作$\indexmath[Sigma vdash varphi]{\Sigma\vdash\varphi}$.

特别地, 如果$T$是语句集, 而$\sigma$是语句, 如果$\indexmath[T vdash sigma]{T\vdash\sigma}$, 就称存在从$T$到$\sigma$的一个\indexbf{证明}.
\end{definition}
...
如所有集合的类$\indexmath[V]{\properclass{V}}$就是一个真类 (根据Corollary \ref{Russell set}). 
...
\end{align}
这样的$Y$称为$X$的\indexbf{幂集}, 记为$\indexmath[P X]{\mathscr{P}(X)}$或$\indexmath[2 X]{2^X}$.
\end{axiom}

\begin{definition}
对任意集合$x$, $x\cup \{ x\} $称为其\indexbf{后继}, 记为$\indexmath[S x]{S(x)}$或$\indexmath[x plus]{x^+}$.
\end{definition}
...
\section{关系}
\begin{definition}集合$a,b$的\indexbf{有序对}$\indexmath[a b]{( a,b)} := \big\{\{a\},\{a,b\}\big\}$.
\end{definition}
...
\begin{align}
	\indexmath[X times Y]{X\times Y} := \{ ( x,y) \mid x\in X\wedge y\in Y\}.
\end{align}
简记$X\times X =: \indexmath[X 2]{ X^2}$.
\end{definition}
...
\begin{definition}
如果存在集合$X,Y$ s.t.\ $R\subseteq X\times Y$, 则称集合$R$是\indexbf{二元关系}. 通常记$(x,y)\in R =: \indexmath[R x y]{R(x,y)}X$, 或$\indexmath[x R y]{xRy}{xRy}$. $\dom R := \{ x \mid \exists y R(x,y)\}$称为其\indexbf{定义域}, $\ran R=\{ y\mid \exists x R(x,y)\}$称为其\indexbf{值域}. 
...
\begin{definition}
如果$X$上的关系$\indexmath[preceq]{\preceq}$只满足传递和自反, 称其为$X$上的\indexbf{拟序} (quasi-order)或\indexbf{预序} (preorder). $\succeq:=\preceq^{-1}$.
\end{definition}
...
\begin{definition}
\begin{align*}
	\indexmath[N]{\mathbb{N}} :=\{n\mid \forall X(X\text{ 是归纳集} \to n\in X)\}.
\end{align*}
