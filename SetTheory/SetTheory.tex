...
\begin{theorem}[\indexbf{Cantor-Bernstein-Schr\"oeder 定理}]
	\label{thorem:CBS}
...
\begin{definition}[有穷与无穷]
	设 $X$ 是一个集合.
	\begin{conditionlist}[label=(\arabic*)]
		\item 倘若 $\exists n \in \mathbb N$ s.t.\ $|X| = n$, 则称 $X$ 是\indexbf{有穷的}.
		\item 倘若 $\forall n \in \mathbb N$, $|X| \neq n$, 则称 $X$ 是\indexbf{无穷的}.
		\item 倘若 $|X| = |\mathbb N|$, 就称 $X$ 是\indexbf{可数的}或\indexbf{可数无穷的}.
		\item 有穷或可数的集合称为\indexbf{至多可数的}.
		\item 不是可数的无穷集合称为\indexbf{不可数的}.
	\end{conditionlist}
\end{definition}
...
\begin{proof}
	...
	其次, 假设命题对 $n \in \mathbb N$ 成立, 但对 $n + 1$ 不成立, 那么 $\exists m \in \mathscr P(n + 1)$, 即 $m \leq n + 1$, 使得 $|m| = m = n + 1 = |n|$. 
	考虑到 Lemma~\ref{lemma:CBS}, 因 $m \subseteq n \subseteq n + 1$, 而 $|m| = |n|$, 所以 $|n| = |m|$, 与假设矛盾.
\end{proof}
