...
\begin{align}
	\indexfm{X\times Y}{X_times_Y} := \{ ( x,y) \mid x\in X\wedge y\in Y\}.
\end{align}
简记$X\times X =: \indexfm{ X^2}{X2}$.
\end{definition}

\begin{theorem}对于$\forall X\forall Y$, $X\times Y$是集合.
\end{theorem}
\begin{proof}
令$\varphi(z) = \exists x\in X\exists y\in Y( (x,y)=z)$, 取$Z=\{z\in 2^\{X\cup Y\}\mid \varphi(z)\}$, 由Ext和Sep (Axion \ref{Sep}) 即可知$X\times Y = Z$.
\end{proof}

\begin{definition}
如果存在集合$X,Y$ s.t.\ $R\subseteq X\times Y$, 则称集合$R$是\indexbf{二元关系}. 通常记$(x,y)\in R =: \indexfm{R(x,y)}{R(x,y)}$, 或$\indexfm{xRy}{xRy}$. $\dom R := \{ x \mid \exists y R(x,y)\}$称为其\indexbf{定义域}, $\ran R=\{ y\mid \exists x R(x,y)\}$称为其\indexbf{值域}. 
...
\begin{definition}
令$f\colon X\to Y$是函数. 若$f(x_1) = f(x_2) \leftrightarrow x_1=x_2$则称$f$为\indexbf{单射}. 若$\ran f = Y$则称其为满射. 既单又满的函数称为双射. 如果函数的逆$f^{-1}$也是函数, 则函数$f$称为\indexbf{可逆}的.
\end{definition}
...
记$\indexfm{Y^X}{YX} := \{ f \mid f\colon X\to Y\}$. $\forall Y\big(Y^\varnothing = \{\varnothing_Y\}\big)$, 而若$X\neq \varnothing$, $\varnothing^X=\varnothing$.
\begin{theorem}
...
\begin{definition}
令$X$是一集合, $S\subseteq 2^X$. $S$被称为$X$的一个\indexbf{划分}如果:
\begin{align*}
...
如果这样的下界$a$的集合有最大元$a_0$, 称其为\indexbf{下确界}, 记为$\inf X_0$. 同理有\indexbf{上界}和\indexbf{上确界}$\sup X_0$.
\end{definition}

作为例子, 令$X\neq \varnothing$, $(2^X, \subseteq)$是偏序集. 对于$\forall S\subseteq 2^X$, $S$有上下确界, $\sup S = \cup S$, $\inf S = \cap S$.

\begin{definition}
如果$X$和其上的线序$(X,\leq)$满足任意$X_0\in 2^X$, $X_0$都有最小元, 则称$\leq$为$X$上的\indexbf{良序}, $X$被称为\indexbf{良序集}.
\end{definition}
...
\begin{definition}
\begin{align*}
	\indexfm{\mathbb{N}}{N} :=\{n\mid \forall X(X\text{ 是归纳集} \to n\in X)\}.
\end{align*}
