...
\begin{definition}[Exterior product]
	Let $\mathscr A$, $\mathscr B$, $\mathscr C$ be multilinear skew-symmetric forms, of degrees $a$, $b$ and $c$. The \indexbf{exterior product} $\indexmath[A wedge B]{\mathscr A \wedge \mathscr B}$ is defined as a $(a + b)$-linear skew-symmetric forms, and satisfies that:
	\begin{conditionlist}
...
\begin{definition}[Differential $p$-form]
	Let $D \subset \mathbb R^n$ (or $\mathbb C^n$) be an open, connected and bounded set in $\mathbb R^n$. 
	If we assign each point $\bv x \in D$ a skew-symmetric $p$-form $\omega(\bv x) \colon (\mathrm T_{\bv x} D)^p \to \mathbb R$ (or $\mathbb C$), we say that we have a real (or complex) \indexbf{differential $p$-form} $\omega$.
\end{definition}
...
Linear algebra would tell us, if we choose a set of bases $\basis e_i$, $i \in n$, the skew-symmetric $k$-form $\omega$ would satisfies:
\begin{align*}
	\omega(\bv x_0, \ldots, \bv x_{k-1}) 
	&= \sum_{0 \leq i_0 < \cdots < i_{k-1} < n} \omega(\basis e_0, \ldots, \basis e_{k-1}) \det((\bv x_j)_{i_\ell})_{k, \ell \in k}\\
	&= \sum_{0 \leq i_0 < \cdots < i_{k-1} < n} a_{i_0, \ldots, i_{k-1}} \bigwedge_{\ell \in k} \dif \bv x_{i_k} (\bv x_0, \ldots, \bv x_{k-1}),
\end{align*}
where $\bv x_j = \sum_{i \in n} (x_j)_i \basis e_i$. 
