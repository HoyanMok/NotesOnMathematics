...
\begin{theorem}
	...
	$f \in C(E, Y)$, $g \in C(f(E), Z)$, then $g \circ f \in C(E, Z)$.
\end{theorem}

\begin{theorem}\label{theorem: continuous iff oscillation is zero}
...
\begin{definition}[Differentiation]
	...
	Such $\mathscr L(\bv x)$ is called the \indexbf{diffrential} of $f$ at $\bv x$\footnote{Alternatively, \indexbf{tangent mapping} or \indexbf{dirivative}.}, denoted by $\indexfm[df x]{\dif f (\bv x)}$ or $\indexfm[f prime x]{f'(\bv x)}$.
\end{definition}

\begin{theorem}[Uniqueness]
	Let $X$ and $Y$ be two normed spaces. 
	If a mapping~$f \in Y^D$ where $D \in \mathscr P(X)$ is diffentiable at $\bv x$ which is an interior point of $D$, then the differential of $f$ at $\bv x$ is unique.
\end{theorem}
\begin{proof}
