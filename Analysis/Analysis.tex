...
\AtBeginEnvironment{pmatrix}{\everymath{\displaystyle}} % 强制矩阵中使用 display 模式
...
\begin{definition}[Diffeomorphism]
	...
	\begin{conditionlist}
		...
		\item $f^{-1} \in C^{(p)}(V)$,
	\end{conditionlist}
	then we call $f$ a \indexbf{$C^{(p)}$-diffeomorphism} or a \indexbf{diffeomorphism with smoothness $p$}.
\end{definition}
...
We use the following notation to denote one of the \emphbf{iterated integrals}\index{iterated integral}:
\begin{equation*}
...
To get rid of some boundary problem, we want the formula to be apllied on some compact set, however a linear approximation is only valid in an open set, hence we introduce: % I do not understand yet
...
\begin{definition}[Elementary diffeomorphism]
	An \indexbf{elementary diffeomorphism}~$\varphi \colon D_t \mapsto D_x$ is defined as a diffeomorphism, being a change of one of the coordinates in a given basis, i.e.\ 
	\begin{equation*}
...
\begin{theorem}[Change of variables for elementary diffeomorphism]
...
\begin{theorem}[Change of variables for composite diffeomorphism]
...
\begin{definition}[$k$-D surface]
	A set $S \subset \mathbb R^n$ is called a \indexbf{$k$-D surface} if $\forall \bv x \in S$, $\exists U \in \mathscr U(\bv x)$ s.t.\ $U \cap S$ is homeomorphic to $\mathbb R^k$.
\end{definition}
...
\begin{definition}[Chart]
	...
	Given a homeomorphism~$\varphi \colon \mathbb R^k \to U$ (or from any parameterised topological space that is homeomorphic to $\mathbb R^k$, e.g.\ a ball or a (non-degenerate) cuboid in $\mathbb R^k$), the pair~$(U, \varphi)$, is called a \indexbf{chart} or \indexbf{local chart} of $S$. 
	$\mathbb R^k$ is called the \indexbf{parametre domain} of the chart and $U$ is called the \indexbf{range of action} or \indexbf{domain of action} of the chart on $S$. 
\end{definition}
...
\begin{definition}[Smooth surface]
	...
	If there is an atlas $\{(U_i, \varphi_i) \mid i \in \mathbb N\}$ of $S$ s.t.\ $\varphi_i \in C^{(m)}(U_i)$ ($m \in \mathbb N_+$) and $\forall \bv x \in U_i$, $\rank \varphi_i'(\bv x) = k$, we shall call $S$ a \indexbf{smooth surface} (of class~$C^{(m)}$).
\end{definition}

Surface can be given by either an atlas or a group of equations:
\begin{theorem}[Surface given by equations]
...
\begin{definition}[Orientation classes of frames]
	Two bases are said to belong one \indexbf{orientation class of frames} if the transform matrix between them has a positive determinent.
\end{definition}
...
\begin{definition}[Consistence of two charts]
	Let $S$ be a $k$-D surface, $(\varphi_i, U_i)$, $i \in 2$ be two charts of $S$. 
	If $\forall \bv x \in U_0 \cap U_1$, $\det \varphi'_0(\bv t_0) \det \varphi'_1(\bv t_1) > 0$ (i.e.\ with a same sign) where $\bv t_i := \varphi^{-1}_i(\bv x)$, $i \in 2$, then we say that these two charts are \emphbf{consistent}\index{consistence of two charts}.
\end{definition}

\begin{definition}[Orienting atlas and orientable surface]
	If an atlas is made up of charts consistent pairwisely, we say that it is an \indexbf{orienting atlas}.

	If a surface has an orienting atlas, we say that it is an \indexbf{orientable surface}. 
	Otherwise we say that it is an \indexbf{nonorientable surface}.
\end{definition}

\begin{definition}[Orientation class of atlases]
	...
	Such equivalence relation induces an equivalence class, called \indexbf{orientation class of atlases}.

	A surface with a certain class of atlases is said to be \emphbf{oriented}\index{oriented surface}, or we say that we have a orientation of the surface.
\end{definition}
...
\begin{theorem}[Conservation of orientability under diffeomorphism]
	Let $\varphi \colon C \to C'$ be a diffeomorphism, $S \subset C$ is an orientable surface. 
	$S' := \varphi(S)$ is also an orientable surface.
\end{theorem}
\begin{proof}
	% TODO Conservation of orientability under diffeomorphism
\end{proof}

\begin{definition}[Normal vector]
	Let $S$ be a $k$-D surface in $\mathbb R^n$, $\bv x \in S$. 
	If a non-zero vector $\basis n \in \mathrm TS_{\bv x}^{\bot}$ i.e.\ the inner product of $\basis n$ and any vector in $\mathrm TS_{\bv x}$ is zero, then we say that $\basis n$ is a \indexbf{normal vector} of $S$ at $\bv x$.
\end{definition}
...
\end{theorem}
\begin{proof}
	% TODO orientation given by normal vector
\end{proof}
...
\begin{definition}[Surface with boundary]
	...
	The collection of all boundary point of $S$ is said \emphbf{the boundary of the surface} $S$\index{boundary of a surface}, denoted by $\indexmath[partial S]{\partial S}$.
\end{definition}
...
As Definition~\ref{definition: smooth surface}, we can define $C^{(m)}$ \emphbf{smooth surface with boundary}, but only to limit the calculation of partial derivative within $H_k$. % TODO: make the def of smooth surface with boundary clear
...
\begin{theorem}[The orientation of the boundary of a smooth surface]
...
\begin{definition}[Consistency of orientation]
	...
	The orientation of $\partial S$ given by the orienting atlas $A(\partial S) := \{(\mathbb R^{k-1}, \varphi_i|_{\mathbb R^{k-1}}, \partial U_i \cap U_i) \mid i \in I\}$ is said to be \indexbf{consistent with the orientation of the surface}.
\end{definition}
...
\begin{definition}[Piecewise-smooth surface]
	...
	If a $k$-D surface $S$ in $\mathbb R^n$ can become a union of several smooth surfaces $S_i$ ($i \in \mathbb N$) after removing countable amount of piecewise-smooth surfaces of dimension at most $k-1$ from it, we say that $S$ is a \indexbf{piecewise-smooth surface}.
\end{definition}
...
\begin{definition}[Orientable piecewise-smooth surface]
	...
	we say $S$ is an \indexbf{orientable piecewise-smooth surface}.
\end{definition}
...
\begin{definition}{Metric tensor}
	...
	The \indexbf{metric tensor} at a point $\bv t \in D$ is given by:
	\begin{equation*}
...
\begin{definition}[Area of surface]
	...
	If $\varphi_i \colon D_i \to S_i \in C^{(1)}(D)$, where $D_i$ is a bounded, connected open set in $\mathbb R^k$, we define the \emphbf{area of the surface}\index{area of surface} $S$ as:
	\begin{equation*}
...
\begin{theorem}[Area is independent of frames]
...
\begin{definition}[Zero measure on a surface]
	...
	we say that $E$ is of \indexbf{$k$-D zero measure} or $E$ has \indexbf{zero Lebesgue area}.
\end{definition}
...
We denote the \indexbf{tensor product} of a $k$-form and a $\ell$-form by $\indexmath[otimes]{\otimes}$: 
...
We can define a \indexbf{skew-symmetrisation operator}:
...
\begin{definition}[Exterior product]
	$\mathscr A \in \varOmega^k$, $\mathscr B \in \varOmega^\ell$, the \indexbf{exterior product} of $\mathscr A$ and $\mathscr B$ is defined:
	\begin{equation*}
...
\begin{definition}[Adjoint mappings]
	...
	The \indexbf{adjoint mapping} $\indexmath[L ast]{\mathscr L^*}$ is defined as:
	\begin{equation*}
...
\begin{definition}[Tangent space]
	...
	The \indexbf{tangent space} $\indexmath[T x S]{\mathrm T_{\bv x} S}$ of $S$ at point $\bv x = \varphi_i(\bv t) \in S$ is defined as:
	\begin{equation*}
		\mathrm T_{\bv x} S := \varphi'_\alpha(\bv t) [\mathrm T_{\bv t} \varphi_\alpha^{-1}(U_i)]. 
	\end{equation*}

	An element in the tangent space is a \indexbf{tangent vector}. 
	The union of all tagent space of $S$ is called the \indexbf{tangent bundle} of the surface $S$, denoted by $\indexmath[T(S)]{\mathrm T(S)}$, which can be considered as the collection of $(\bv x, \bv v)$, where $\bv x \in M$, $\bv v \in \mathrm T_{\bv x} S$.
\end{definition}
...
\begin{definition}[Cotangent space]
	...
	The co-space of $T_{\bv x} S$ is called the \indexbf{cotangent space} of $S$ at $\bv x \in S$, denoted by $\indexmath[T ast x S]{T^*_{\bv x} S}$.
\end{definition}
...
\begin{definition}[Differential form]
	...
	If we assign a $\omega(\bv x) \in \varOmega^p(\mathrm T_{\bv x} S)$ for any $\bv x$, we say that we have defined a \indexbf{differential $p$-form} $\omega$. 
...
For example, if we define a force field $\bv F \colon D \to \mathbb R^n$, naturally a \indexbf{work form of the field} $\bv F(\bv x)$ can be defined as:
\begin{equation*}
	W_{\bv F} (\bv x) (\Delta \bv x) := \langle \bv F(\bv x), \Delta \bv x \rangle.
\end{equation*}

Or, if we define a velocity field $\bv V \colon D \to \mathbb R^n$, a \indexbf{flow form of the field} $\bv V(\bv x)$ can be defined as:
\begin{equation*}
...
\begin{definition}[Smooth differential form]
	If the coordinate expression of differential form $\omega$ is given by Eq.~\ref{equation: coordinate expression of differential form}, and $\forall \alpha \in A$, $\forall \bv x \in U_\alpha$, $a_{\alpha; i_0 \cdots i_{p-1}}$ is a smooth function (of class $C^{(m)}$), then we call $\omega$ a \indexbf{smooth differential form} (of class $C^{(m)}$).
...
\begin{definition}[Exterior differentiation]
	...
	\end{conditionlist}
	then we call $\dif$ the \indexbf{exterior differentiation}.
\end{definition}
...
% TODO: Mean-Value Inequality
\section{Cauchy's Inequality}
% TODO: Cauchy's Inequality
\section{Jensen's Inequality}
% TODO: Jensen's Inequality
\section{Brunn-Minkowski Inequality}
% TODO: Brunn-Minkowski Inequality
\section{H\"older's Inequality and Minkowski's Inequality}
% TODO: H\"older's Inequality and Minkowski's Inequality
