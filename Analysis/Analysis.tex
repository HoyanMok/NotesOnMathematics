...
\begin{definition}[Continuously differentiable]
	...
	We can denote all continuously differentiable mappings from an open set $X$ to $Y$ by $\indexfm[C 1 X Y]{C^{(1)}(X, Y)}$\footnote{or $\indexfm[C 1 X]{C^{(1)}(X)}$ if you are sure about what $Y$ is.}. 
\end{definition}
...
\begin{definition}[$n$-th differentiation]
	...
	we can define the $(n + 1)$-th differential $\indexfm[f n x]{f^{(n)}(\bv x)}$ by:
	\begin{equation*}
		f^{(n + 1)}= (f^{(n)})'\,. 
	\end{equation*}
\end{definition}
...
\begin{proof}
	...
	Substituting $(\bv \ell_0, \bv \ell_1)$ by $(\bv \ell_1, \bv \ell_1)$ in the definition of $\Delta(t)$ doesn't change its value, hence we have proved the theorem in the case when $n = 2$.
\end{proof}
