...
\begin{definition}[Box]
	...
	The \emphbf{boxes}\index{box}~$I_{\bv a, \bv b}$ in $X := \prod_{i \in n} X_i$, where $\bv a, \bv b \in X$, are defined as:
	\begin{equation*}
		I_{\bv a, \bv b} 
...
Before our definition of volume of subsets of $X$, we discuss on the volume of boxes. 
The volume, or, the measure of the boxes shall be like:
\begin{equation}
	\label{equation: measure of box}
	\mu(I_{\bv a, \bv b}) = \prod_{i \in n} \|a_i - b_i\|_i
\end{equation}

If a (countable) collection of boxes are pairwise disjoint i.e.\ in which each two boxes are disjoint, we shall expect their union has a volume:
\begin{equation*}
	\mu \left( 
...
If there are a collections of boxes $\{I_i\}_{i \in n}$ that covers the given box $I$, we shall see:
\begin{equation*}
...
As an example, the $\sigma$-algebra closure of boxes (The intersections of all $\sigma$-algeras containing all boxes) is called the \indexbf{Borel sets}.
...
\begin{definition}[Lebesgue outer measure]
	...
	\begin{equation*}
		...
		\right\}\,,
	\end{equation*}
	where the volume of the boxes~$\mu$ is defined as Eq.~\eqref{equation: measure of box}.
\end{definition}
...
\end{theorem}
\begin{proof}
	If $\{I_i\}_{i \in \mathbb N}$ cover $B$, then they must cover $A$.
\end{proof}
...
\end{theorem}
\begin{proof}
	$\forall \varepsilon \in \mathbb R_+$, by the definition of infimum, for each $k \in \mathbb N$, find a sequence of boxes~$\langle I^{(k)}_i \rangle_{i \in n}$ that covers $A_k$, and:
	\begin{equation*}
...
\end{theorem}
\begin{proof}
	For each set $E_k$, find it a cover with boxes that the sum of the measure of the boxes are less than $\varepsilon / 2^k$.
\end{proof}
...
\begin{lemma}
	\label{lemma: zero measure defined by open boxes}
	...
	Here $I'_i$ are open boxes, defined by products of $n$ open intervals. 
\end{lemma}
\begin{proof}
	For any $\varepsilon$ we multiply it by $\lambda^n$ where $\lambda < 1$, the definition yield that we can find a sequence of boxes, the sum of the measure of which is less than $\lambda^n \varepsilon$.
	We extend the boxes by $\lambda^{-1}$, we see that the interior point of which contain the previous boxes.
\end{proof}

\begin{theorem}
	\label{theorem: compact zero measure defined by finite boxes}
...
\end{theorem}
\begin{proof}
	By Lemma~\ref{lemma: zero measure defined by open boxes}, we can find an open cover with measure less than $\varepsilon$ of $E$, and therefore there is a finite subcover. 
	The measure of the subcover is of course less than that of the cover. 
\end{proof}

\section{\texorpdfstring{Riemann Integral on $n$-D boxes}{Riemann Integral on n-D boxes}}

Now we introduce the partition of the box: 
\begin{definition}[Partition of a box]
	A \indexbf{partition}~$P$ of a box~$I_{\bv a, \bv b}$, is defined as a \emph{finite} collection of boxes which have no common interior point pairwisely, and the union of which is the box itself $I_{\bv a, \bv b}$.
\end{definition}

\begin{definition}[Mesh]
	The \indexbf{mash} of a partition $P$ is the maximum diametre of the boxes in $P$:
	\begin{equation*}
...
All partitions of a box $I$ is denoted by $\mathfrak P(I)$.
...
\begin{definition}[Riemann sum]
	...
	Let $I$ be a box in $X$, $f \in Y^I$, $(P, \bv \xi) \in \mathfrak P(I)$. 
...
We say a proposition $p(x)$ holds \indexbf{almost everywhere} on $X$, meaning $\exists E \subset X$, s.t.\ $E$ is of measure zero and $\forall x \in (X - E) \big(p (x)\big)$. 

\begin{theorem}[Lebesgue's criterion]
	Let $f \in \mathbb R^I$, where $I$ is a box.
	$f \in \mathfrak R(I)$ $\IFF$ $f$ is bounded in $I$ and $f$ is almost continuous on $I$.
\end{theorem}
\begin{proof}
