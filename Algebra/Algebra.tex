...
\begin{definition}[最小倍]
	...
	\end{enumerate*} 
	则 $d$ 被称为 $a$, $b$ 的一个\indexbf{最小倍} (least common multiple), 记为 $\indexfm[lcm a b]{\lcm (a, b)}$. 
\end{definition}
...
由于 $Q(A)$ 的子环 $[a, 1]$ 与 $A$ 同构, 我们通常也直接说 $A \subset Q(A)$. 同时, 习惯上我们记 $[a, b] = \indexfm[a / b]{a / b}$, 甚至 $1/a^n =: a^{-n}$. 注意, $a^{-n} \in Q(A)$ 而不一定在 $A$ 中 (除非 $a \in U(A)$).
...
\begin{definition}[本原多项式]
	...
	~$\indexfm[c P]{c(P)}$ 叫做多项式的\indexbf{容度} (content). 
	如果 $c(P) \in U(R)$, 那么 $P(X)$ 被称为一个\indexbf{本原多项式} (primitive polynomial). 
	一个非零多项式 $P(X)$ 所有系数对 $c(P)$ (由最大因的定义, 它们都是容度的倍数) 的商为系数的多项式 $\indexfm[pp P X]{\pp P(X)}$ 称为该多项式的\indexbf{本原部分} (primitive part).
\end{definition}

\begin{definition}
	我们通常称域~$\mathbb F$ 上的多项式环~$\mathbb F[X]$ 的分式域~$Q(\mathbb F[X])$ 为\indexbf{有理函数域}, 记为 $\indexfm[F X]{\mathbb F(X)}$. 
\end{definition}
