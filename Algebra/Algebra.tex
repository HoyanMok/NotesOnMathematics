...
\begin{definition}[代数]
	...
	\end{equation*}
	那么我们称 $A$ 是 $\mathbb F$ 上的一个\indexbf{代数} (algebra)\footnote{实际上这里是结合的, 有单位元的代数. 如何处理术语请容我再思考思考.}.
...
元素全为 $0$ 的矩阵记为 $\indexfm[O]{\bv O}$, 有时为了强调它的尺寸, 将之写在右下角 i.e.\ $\indexfm[O m times n]{O_{m \times n}}$.
...
\begin{definition}[对角矩阵]
	若方阵~$\bv A$ 的元素只有对角线上的元素非零 i.e.\ $ a_{ij} \neq 0  \IFF i = j$, 称其为\indexbf{对角矩阵}, 记为 $\indexfm[diag a ii i in n]{\diag(a_{ii})_{i \in n}}$. 特别地 $\indexfm[I]{\bv I} := \diag(1)_{i \in n}$ 称为 $n$~维\indexbf{单位阵}.
\end{definition}
...
\begin{definition}[多项式环]
	...
	\begin{equation*}
		...
		\left\{ 
			...
			\middle|
			\forall i \in \mathbb N \big(
					p_i \in R \big)
			,\,
			\exists N \in \mathbb N\,\forall n \in \mathbb N - N \big(
					p_n = 0\big)
		\right\}
	\end{equation*}
	上定义了加法:
