...
若 $\bv v$ 在基底 $\hat e = (\basis e_i)_{i \in n}$ 下的坐标为 $(v_i)_{i \in n}$, 记之为 $\left. \bv v \right|_{\hat e}$. 
...
那么, 
\begin{equation*}
	\left. \bv v \right|_{\hat e} = \left( 
		\sum_{j \in n} a_{ij} v'_j 
 \right)_{i \in n}
\end{equation*}
或 $\left. \bv v \right|_{\hat e} = \bv A \left. \bv v \right|_{\hat f}$. 同理 $\left. \bv v \right|_{\hat f} = \bv A^{-1} \left. \bv v \right|_{\hat e}$. 
...
\begin{equation*}
	\ell \colon \bv x \mapsto \left.\bv \ell\right|_{\hat e} \left. \bv x\right|_{\hat e}\,,
\end{equation*}
其中 $\left.\bv \ell\right|_{\hat e}$ 是 $1 \times n$ 的行向量. 坐标变换到 $\hat f$ 时, 设转换矩阵是 $\bv P$, 那么:
\begin{equation*}
...
\begin{definition}[对偶空间]
	线性空间 $V$ 上所有的线性型构成线性空间 $\indexfm[V ast]{V^*}$, 称为 $V$的\indexbf{对偶空间} (dual space), 线性组合和零元已定义如前. 通常对偶空间的元素可称为\indexbf{余向量}, 或\indexbf{共变向量} (与此同时, $V$ 的元素对应地称为\indexbf{反变向量}).
\end{definition}

为区别两种向量, 有用 $x^i$ 表示反变向量而用 $\ell_i$ 表示共变矢量, 并引入Einstein求和约定的, 见之后第\ref{chapter:张量}章.
