...
作为例子, $(\mathbb Z, +, \cdot)$ 是我们熟悉的\indexbf{整环}, $n\mathbb Z := \{nk \mid k \in \mathbb Z\}$ 是它的一个子环 ($n \in \mathbb Z$). 
...
考虑到将$\lbrack k\rbrack _n \in \mathbb Z / \equiv \bmod n $ 映射到 $n^\mathbb Z \ni \bmod n := \{(n, k) \in \mathbb Z \times n \mid n \equiv k \bmod n\}$ 的同构, 
\indexbf{模$n$的剩余类环}\index{剩余类环} $(\mathbb Z_n, +, \cdot)$ 即是可看作函数环 $n^\mathbb Z$ 的一个交换子环, 其中$\mathbb Z_n := \{\lbrack k\rbrack_n \mid k \in n\}$. 
\appendix
