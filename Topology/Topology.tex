...
\subsection{Topological Space and Continuous Mapping}
\begin{definition}\label{topological_space}
We say $X$ is equiped with a \indexbf{topological space} or epuiped with \indexbf{topology} if we assigned a $\indexfm{\mathscr{T}}{T}\subset 2^X$, which has got the following propoties:
\begin{enumerate}[a)]
...
\begin{definition}\label{continuouseness}
Let $(X; \mathscr T)$, $(Y; \mathscr S)$ be two topological space. A mapping $f$ is said to be continuous if
\begin{align*}
