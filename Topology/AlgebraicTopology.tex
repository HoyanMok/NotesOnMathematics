...
In this section, we only talk about Abelian groups, and their multiplications are called ``addition''.
...
\begin{definition}[Finitely generated group]%
    \label{definition: finitely generated group}
    $(F, +)$ is an Abelian group.
    If $\exists A \subset F$ s.t.\ $\card A \in \mathbb Z$ and $\forall f \in F$, $\exists n_f \colon A \to \mathbb Z$ s.t.\ 
    \begin{equation*}
        f = \sum_{a \in A} n_f(a) a,
    \end{equation*}
    then $F$ is called a \indexbf{finitely generated group}, $A$ \emphbf{generates} $F$\index{generate}.
\end{definition}
...
\begin{definition}
    Let $E$, $B$ be two topological spaces, $p \in C(E, B)$.
    If $\forall b in B$, $\exists U \in \mathscr U(b)$ s.t.\ $p^{-1}(U) = \coprod_{i \in I} U_i$ and $\forall i \in I$, $p|_{U_i}$ is a homeomorphism from $U_i$ to $U$,
